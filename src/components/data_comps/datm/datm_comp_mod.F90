...
! !USES:
  ...
  ! anomaly forcing
  integer(IN) :: sprecsf,sprec_af,su_af,sv_af,stbot_af,sshum_af,spbot_af,slwdn_af,sswdn_af
  ...
  integer(IN) :: ktrans                      ! size of avofld and avifld
  integer(IN) :: ktranss                     ! size of stofld and stifod

  character(len=16), allocatable:: avofld(:) ! array of driver_fld names
  character(len=16), allocatable:: avifld(:) ! array of datm_fld names that map to driver_fld names
  character(len=16), allocatable:: stofld(:) ! array of internal stream_fld names that map to datm_fld_names
  character(len=16), allocatable:: stifld(:) ! array of datm_fld names used for stream_fld_names
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  subroutine avfld_set(count_only)
    ...
    call avfld_add(avifld, avofld, index, count_only, datm_fld="z",         driver_fld="Sa_z"           )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="topo",      driver_fld="Sa_topo"        )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="u",         driver_fld="Sa_u"           )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="v",         driver_fld="Sa_v"           )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="tbot",      driver_fld="Sa_tbot"        )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="ptem",      driver_fld="Sa_ptem"        )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="shum",      driver_fld="Sa_shum"        )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="dens",      driver_fld="Sa_dens"        )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="pbot",      driver_fld="Sa_pbot"        )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="pslv",      driver_fld="Sa_pslv"        )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="lwdn",      driver_fld="Faxa_lwdn"      )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="rainc",     driver_fld="Faxa_rainc"     )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="rainl",     driver_fld="Faxa_rainl"     )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="snowc",     driver_fld="Faxa_snowc"     )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="snowl",     driver_fld="Faxa_snowl"     )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="swndr",     driver_fld="Faxa_swndr"     )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="swvdr",     driver_fld="Faxa_swvdr"     )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="swndf",     driver_fld="Faxa_swndf"     )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="swvdf",     driver_fld="Faxa_swvdf"     )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="swnet",     driver_fld="Faxa_swnet"     )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="co2prog",   driver_fld="Sa_co2prog"     )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="co2diag",   driver_fld="Sa_co2diag"     )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="bcphidry",  driver_fld="Faxa_bcphidry"  )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="bcphodry",  driver_fld="Faxa_bcphodry"  )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="bcphiwet",  driver_fld="Faxa_bcphiwet"  )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="ocphidry",  driver_fld="Faxa_ocphidry"  )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="ocphodry",  driver_fld="Faxa_ocphodry"  )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="ocphiwet",  driver_fld="Faxa_ocphiwet"  )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="dstwet1",   driver_fld="Faxa_dstwet1"   )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="dstwet2",   driver_fld="Faxa_dstwet2"   )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="dstwet3",   driver_fld="Faxa_dstwet3"   )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="dstwet4",   driver_fld="Faxa_dstwet4"   )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="dstdry1",   driver_fld="Faxa_dstdry1"   )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="dstdry2",   driver_fld="Faxa_dstdry2"   )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="dstdry3",   driver_fld="Faxa_dstdry3"   )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="dstdry4",   driver_fld="Faxa_dstdry4"   )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="tref",      driver_fld="Sx_tref"        )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="qref",      driver_fld="Sx_qref"        )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="avsdr",     driver_fld="Sx_avsdr"       )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="anidr",     driver_fld="Sx_anidr"       )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="avsdf",     driver_fld="Sx_avsdf"       )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="anidf",     driver_fld="Sx_anidf"       )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="ts",        driver_fld="Sx_t"           )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="to",        driver_fld="So_t"           )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="snowhl",    driver_fld="Sl_snowh"       )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="lfrac",     driver_fld="Sf_lfrac"       )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="ifrac",     driver_fld="Sf_ifrac"       )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="ofrac",     driver_fld="Sf_ofrac"       )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="taux",      driver_fld="Faxx_taux"      )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="tauy",      driver_fld="Faxx_tauy"      )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="lat",       driver_fld="Faxx_lat"       )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="sen",       driver_fld="Faxx_sen"       )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="lwup",      driver_fld="Faxx_lwup"      )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="evap",      driver_fld="Faxx_evap"      )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="co2lnd",    driver_fld="Fall_fco2_lnd"  )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="co2ocn",    driver_fld="Faoo_fco2_ocn"  )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="dms",       driver_fld="Faoo_fdms_ocn"  )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="precsf",    driver_fld="Sa_precsf"      )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="prec_af",   driver_fld="Sa_prec_af"     )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="u_af",      driver_fld="Sa_u_af"        )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="v_af",      driver_fld="Sa_v_af"        )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="tbot_af",   driver_fld="Sa_tbot_af"     )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="pbot_af",   driver_fld="Sa_pbot_af"     )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="shum_af",   driver_fld="Sa_shum_af"     )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="swdn_af",   driver_fld="Sa_swdn_af"     )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="lwdn_af",   driver_fld="Sa_lwdn_af"     )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="rainc_18O", driver_fld="Faxa_rainc_18O" )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="rainc_HDO", driver_fld="Faxa_rainc_HDO" )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="rainl_18O", driver_fld="Faxa_rainl_18O" )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="rainl_HDO", driver_fld="Faxa_rainl_HDO" )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="snowc_18O", driver_fld="Faxa_snowc_18O" )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="snowc_HDO", driver_fld="Faxa_snowc_HDO" )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="snowl_18O", driver_fld="Faxa_snowl_18O" )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="snowl_HDO", driver_fld="Faxa_snowl_HDO" )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="shum_16O",  driver_fld="Sa_shum_16O"    )
    call avfld_add(avifld, avofld, index, count_only, datm_fld="shum_18O",  driver_fld="Sa_shum_18O"    )
    ...
    call stfld_add(stifld, stofld, index, count_only, datm_fld="tbot",      stream_fld="strm_tbot"      )        
    call stfld_add(stifld, stofld, index, count_only, datm_fld="wind",      stream_fld="strm_wind"      )        
    call stfld_add(stifld, stofld, index, count_only, datm_fld="z",         stream_fld="strm_z"         )        
    call stfld_add(stifld, stofld, index, count_only, datm_fld="pbot",      stream_fld="strm_pbot"      )        
    call stfld_add(stifld, stofld, index, count_only, datm_fld="shum",      stream_fld="strm_shum"      )        
    call stfld_add(stifld, stofld, index, count_only, datm_fld="tdew",      stream_fld="strm_tdew"      )        
    call stfld_add(stifld, stofld, index, count_only, datm_fld="rh",        stream_fld="strm_rh"        )        
    call stfld_add(stifld, stofld, index, count_only, datm_fld="lwdn",      stream_fld="strm_lwdn"      )        
    call stfld_add(stifld, stofld, index, count_only, datm_fld="swdn",      stream_fld="strm_swdn"      )        
    call stfld_add(stifld, stofld, index, count_only, datm_fld="swdndf",    stream_fld="strm_swdndf"    )       
    call stfld_add(stifld, stofld, index, count_only, datm_fld="swdndr",    stream_fld="strm_swdndr"    )       
    call stfld_add(stifld, stofld, index, count_only, datm_fld="precc",     stream_fld="strm_precc"     )        
    call stfld_add(stifld, stofld, index, count_only, datm_fld="precl",     stream_fld="strm_precl"     )        
    call stfld_add(stifld, stofld, index, count_only, datm_fld="precn",     stream_fld="strm_precn"     )        
    call stfld_add(stifld, stofld, index, count_only, datm_fld="co2prog",   stream_fld="strm_co2prog"   )      
    call stfld_add(stifld, stofld, index, count_only, datm_fld="co2diag",   stream_fld="strm_co2diag"   )      
    call stfld_add(stifld, stofld, index, count_only, datm_fld="swup",      stream_fld="strm_swup"      )        
    call stfld_add(stifld, stofld, index, count_only, datm_fld="prec",      stream_fld="strm_prec"      )        
    call stfld_add(stifld, stofld, index, count_only, datm_fld="tarcf",     stream_fld="strm_tarcf"     )        
    call stfld_add(stifld, stofld, index, count_only, datm_fld="precsf",    stream_fld="strm_precsf"    )       
    call stfld_add(stifld, stofld, index, count_only, datm_fld="prec_af",   stream_fld="strm_prec_af"   )      
    call stfld_add(stifld, stofld, index, count_only, datm_fld="u_af",      stream_fld="strm_u_af"      )        
    call stfld_add(stifld, stofld, index, count_only, datm_fld="v_af",      stream_fld="strm_v_af"      )        
    call stfld_add(stifld, stofld, index, count_only, datm_fld="tbot_af",   stream_fld="strm_tbot_af"   )      
    call stfld_add(stifld, stofld, index, count_only, datm_fld="pbot_af",   stream_fld="strm_pbot_af"   )      
    call stfld_add(stifld, stofld, index, count_only, datm_fld="shum_af",   stream_fld="strm_shum_af"   )      
    call stfld_add(stifld, stofld, index, count_only, datm_fld="swdn_af",   stream_fld="strm_swdn_af"   )      
    call stfld_add(stifld, stofld, index, count_only, datm_fld="lwdn_af",   stream_fld="strm_lwdn_af"   )      
    call stfld_add(stifld, stofld, index, count_only, datm_fld="rh_18O",    stream_fld="strm_rh_18O"    )       
    call stfld_add(stifld, stofld, index, count_only, datm_fld="rh_HDO",    stream_fld="strm_rh_HDO"    )         
    call stfld_add(stifld, stofld, index, count_only, datm_fld="precn_16O", stream_fld="strm_precn_16O" )   
    call stfld_add(stifld, stofld, index, count_only, datm_fld="precn_18O", stream_fld="strm_precn_18O" )   
    call stfld_add(stifld, stofld, index, count_only, datm_fld="precn_HDO", stream_fld="strm_precn_HDO" )       

    if (count_only) then
       allocate(stifld(index))
       allocate(stofld(index))
       ktranss = index
    end if
  ...
  subroutine datm_comp_init( EClock, cdata, x2a, a2x, NLFilename )
    ...
    ! Determine module arrays avifld, avofld, stifld, stofld
    ! The first call is to get the size to allocate the arrays, the second call is to fill them in

    if (firstcall) then
       call avfld_set(count_only=.true.)
       call avfld_set(count_only=.false.)
    end if

    ! Set cdata pointers

    call seq_cdata_setptrs(cdata, ID=COMPID, mpicom=mpicom, gsMap=gsmap, dom=ggrid, infodata=infodata)
    ...
    if (phase == 1) then
       ...
       call seq_infodata_getData(infodata,single_column=scmMode, scmlat=scmlat, scmlon=scmLon)
       ...
       ! Create a rearranger from the gsmap of the data model grid in SDATM to the
       ! gsmap created above
       if (atm_present) then
       ...
       if(wiso_datm) then
          ...
          ! just use srh_18O and srh_HDO, because the forcing is (should)
  ...
  !===============================================================================
  subroutine avfld_add(avofld, avifld, size, count_only, datm_fld, driver_fld)
    character(len=*), intent(inout) :: avofld(:)
    character(len=*), intent(inout) :: avifld(:)
    integer         , intent(inout) :: size
    logical         , intent(in)    :: count_only
    character(len=*), intent(in)    :: datm_fld
    ...
    if (count_only) then
       ...
       avifld(size) = trim(datm_fld)
       avofld(size) = trim(driver_fld)
    end if
  end subroutine avfld_add

  subroutine stfld_add(stifld, stofld, size, count_only, datm_fld, stream_fld)
    character(len=*), intent(inout) :: stifld(:)
    character(len=*), intent(inout) :: stofld(:)
    integer         , intent(inout) :: size
    logical         , intent(in)    :: count_only
    character(len=*), intent(in)    :: datm_fld
    character(len=*), intent(in)    :: stream_fld
    
    size = size + 1
    if (count_only) then
       return
    else
       stifld(size) = trim(datm_fld)
       stofld(size) = trim(stream_fld)
    end if
  end subroutine stfld_add
