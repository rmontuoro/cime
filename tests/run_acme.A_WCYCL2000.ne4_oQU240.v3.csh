...
set run_name       = A_WCYCL2000_ne4_oQU240_test
...
set fetch_code     = true
set acme_tag       = master
set tag_name       = acme_wcycl2000_cime5
#set acme_tag       = v1.0.0-alpha.8-64-g507b5ee
#set tag_name       = A_WCYCL2000_ne11_test
...
set seconds_before_delete_run_dir    = 0
...
if ( $seconds_before_delete_source_dir >= 0 && `lowercase $old_executable` == true ) then
  echo 'run_acme ERROR: It is unlikely that you want to delete the source code and then use the existing compiled executable.'
...
if ( $seconds_before_delete_bld_dir >= 0 && `lowercase $old_executable` == true ) then
  echo 'run_acme ERROR: It makes no sense to delete the source-compiled code and then use the existing compiled executable.'
  echo '                Hence, this script will abort to avoid making a mistake.'
...
if ( $num_submits >1 && ( $stop_units != $restart_units || $stop_num != $restart_num ) ) then
  echo 'run_acme WARNING: It makes no sense to have chained submissions unless the run is producing appropriate restarts!'
  echo '                  The run length and restarts do not match exactly. '
  echo '                  It is hard to check definitively, so stopping just in case.'
  echo '                  If the settings are OK then deactivate this test.'
  echo '                  $stop_units     = '$stop_units
  echo '                  $stop_num       = '$stop_num
  echo '                  $restart_units  = '$restart_units
  echo '                  $restart_num    = '$restart_num
  echo '                  $num_submits    = '$num_submits
  exit 13
endif

if (   `lowercase $debug_queue` == true && ( $num_submits >1 || `lowercase $do_short_term_archiving` == true ) ) then
  echo 'run_acme ERROR: Supercomputer centers generally do not allow job chaining in debug queues'
  echo '                You should either use a different queue, or submit a single juob without archiving.'
  echo '                $debug_queue             = '$debug_queue
  echo '                $num_submits             = '$num_submits
  echo '                $do_short_term_archiving = '$do_short_term_archiving
  exit 14
endif

if ( `lowercase $do_long_term_archiving` == true ) then
  echo '**************************************************************************************'
  echo 'run_acme WARNING: long_term_archiving does not work.  Deactivating long term archiving'
  echo '                  $do_long_term_archiving = '$do_long_term_archiving
  echo '**************************************************************************************'
  set do_long_term_archiving = false
endif
...
if ( `lowercase $run_root_dir` == 'default' ) then
  if ( $machine == 'edison' || $machine == 'hopper' || $machine == 'cori' ) then
    set run_root_dir = $CSCRATCH/ACME_simulations/${case_name}
  ...
  else if ( $machine == 'sierra' || $machine == 'cab' ) then
    set run_root_dir = /p/lscratche/${USER}/ACME_simulations/${case_name}
  else
...
if ( `lowercase $short_term_archive_root_dir` == 'default' ) then
  if ( $machine == 'edison' || $machine == 'hopper' || $machine == 'cori' ) then
    set short_term_archive_root_dir = $CSCRATCH/archive/${case_name}
  else if (  $machine == 'titan' || $machine == 'eos'  ) then
    ...
    set short_term_archive = /p/lscratche/${USER}/archive/${case_name}
  else
...
echo $create_newcase_exe --case $case_scripts_dir  \
...
$create_newcase_exe --case $case_scripts_dir \
                    --mach $newcase_machine \
                    --compset $compset \
                    --res $resolution \
                    --walltime $walltime \
...
#chgrp $project $case_scripts_dir
...
set run_dir_link = $this_script_name=a_run_link
...
# NOTE: env_build.xml is used by case.setup (despite the name), so set the directories here.
# NOTE: Specifying the file to xmlchange doesn't do anything
$xmlchange_exe --id EXEROOT --val "${case_build_dir}"
$xmlchange_exe --id RUNDIR --val "${case_run_dir}"
...
# CIME 5.1 has a typo in it's env_mach_pes.xml preventing NINST_ICE_LAYOUT from being set to sequential
  $xmlchange_exe --id NINST_ICE_LAYOUT --val equential
...
# These were chosen based on the smallest pe layouts of provided input files
  $xmlchange_exe --id NTASKS_ATM --val 1
  $xmlchange_exe --id NTASKS_LND --val 1
  $xmlchange_exe --id NTASKS_ICE --val 16
  $xmlchange_exe --id NTASKS_OCN --val 4
  $xmlchange_exe --id NTASKS_CPL --val 1
  $xmlchange_exe --id NTASKS_GLC --val 1
  $xmlchange_exe --id NTASKS_ROF --val 1
  $xmlchange_exe --id NTASKS_WAV --val 1

  $xmlchange_exe --id NTHRDS_ATM --val 1
  $xmlchange_exe --id NTHRDS_LND --val 1
  $xmlchange_exe --id NTHRDS_ICE --val 16
  $xmlchange_exe --id NTHRDS_OCN --val 4
...
#PJC#cat <<EOF >> user_nl_cam
#PJC#history_aerosol     = .true.
#PJC#history_amwg        = .true.
#PJC#history_aero_optics = .true.
#PJC#
#PJC#!nhtfrq = $atm_output_freq
#PJC# nhtfrq        = 0,-24,-8760
#PJC# mfilt  = $records_per_atm_output_file
#PJC# fincl1 = 'SFDMS','GS_DMS','AQ_DMS','SFSO2','GS_SO2','AQ_SO2','SFH2SO4','GS_H2SO4','AQ_H2SO4','DMS','SO2','H2SO4','O3',
#PJC#          'o3_clim','t_clim','o3col_clim','PmL_clim','dPmL_dO3','dPmL_dT','dPmL_dO3col','cariolle_pscs',
#PJC#          'LINOZ_DO3','LINOZ_DO3_PSC','LINOZ_SSO3','LINOZ_O3COL','LINOZ_O3CLIM','LINOZ_SZA',     !'ozone',     !'prsd_ch4',
#PJC#          'TROP_P','TROP_T','TROP_Z','TROP_DZ','TROP_PD','TROP_FD','TROPP_P','TROPP_T','TROPP_Z',
#PJC#          'TROPP_DZ','TROPP_PD','TROPP_FD','hstobie_trop','hstobie_linoz','hstobie_tropop',
#PJC#          'PRAO','PRCO','RELVAR','UTGWORO','VTGWORO','UTGWSPEC','VTGWSPEC','BUTGWSPEC','BVTGWSPEC'      !extra fields for Po-Lun Ma
#PJC#! fincl2 = 'O3'    !high frequency output of O3 for animations.
#PJC# fincl3 = 'TMQ','TGCLDCWP','T','PS','Q'
#PJC# avgflag_pertape = 'A','I','I'    !! This is necessary because some of the linoz output fields are Instantaneous by default.
#PJC#! history_aerosol=.true.
#PJC#!!! Grid to interpolate output onto (usually from cubed sphere to rectangular)
#PJC# interpolate_output   =  .true., .true., .false.     ! flag for each output tape.
#PJC# interpolate_gridtype =       1,      1,      1        ! equal angle lat/lon grid, with point at pole (FV scalar grid)
#PJC# interpolate_type     =       1,      1,      1
#PJC# interpolate_nlat     =     257,    257,    257
#PJC# interpolate_nlon     =     512,    512,    512
#PJC#
#PJC#! rad_data_output       = .true.
#PJC#! rad_data_histfile_num =  3
#PJC#! rad_data_avgflag      = 'I'
#PJC#! rad_climate            = 'A:Q:H2O', 'N:O2:O2',
#PJC#!    'N:CO2:CO2', 'A:O3:O3',
#PJC#!    'N:N2O:N2O', 'N:CH4:CH4',
#PJC#!    'N:CFC11:CFC11', 'N:CFC12:CFC12',
#PJC#!    'M:mam4_mode1:/lustre/atlas/proj-shared/cli112/pjcs/ACME_inputdata/atm/cam/physprops/mam4_mode1_rrtmg_c130628.nc',
#PJC#!    'M:mam4_mode2:/lustre/atlas/proj-shared/cli112/pjcs/ACME_inputdata/atm/cam/physprops/mam4_mode2_rrtmg_c130628.nc',
#PJC#!    'M:mam4_mode3:/lustre/atlas/proj-shared/cli112/pjcs/ACME_inputdata/atm/cam/physprops/mam4_mode3_rrtmg_c130628.nc',
#PJC#!    'M:mam4_mode4:/lustre/atlas/proj-shared/cli112/pjcs/ACME_inputdata/atm/cam/physprops/mam4_mode4_rrtmg_c130628.nc'
#PJC#
#PJC#! linoz_data_cycle_yr = 2000
#PJC#! linoz_data_file = 'linoz1800-2100_2006jpl_climo_1.9x2.5_26L_extended.c160204.nc'
#PJC#
#PJC#! ncdata         = '${input_data_dir}/atm/cam/inic/homme/cami_mam3_Linoz_ne30np4_L72_c160214.nc'
#PJC#
#PJC#EOF
...
if ( `lowercase $old_executable` == false ) then
    ...
    if ( -x $case_build_dir/$acme_exe ) then       #use executable previously generated for this case_name.
        ...
        echo '                Expected to find executable = '$case_build_dir/$acme_exe
        exit 297
    endif
else
...
#                        remove support for CIME2 and pre-cime versions
