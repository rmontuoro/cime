...
###############################################################################
    ...
    expect(stat == 0, "ERROR: buildexe failed, cat {}".format(file_build)),
...
###############################################################################
    ...
    for lib in libs:
        ...
        expect(stat == 0, "ERROR: buildlib.{} failed, cat {}".format(lib), file_build)
        logs.append(file_build)
        if lib == "pio":
...
###############################################################################
    ...
    if (stat != 0):
        thread_bad_results.append("ERROR: {}.buildlib failed, cat {}".format(compclass), file_build)
