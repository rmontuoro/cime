...
class NamelistDefinition(EntryID):
    ...
    def split_type_string(self, name):
        ...
        if size_match:
            ...
            try:
                ...
                expect(False,
                       "In namelist definition, variable {} had the "
                       "non-integer string {:r} specified as an array size.".format(name, size_string))
        else:
            size = 1
        ...
        if star == '*':
            ...
            try:
                ...
                expect(False,
                       "In namelist definition, character variable {} had the "
                       "non-integer string {:r} specified as a length.".format(name, length))
        else:
            max_len = None
    ...
    def is_valid_value(self, name, value):
        ...
        if valid_values is not None:
            expect(type_ in ('integer', 'character'),
                   "Found valid_values attribute for variable {} with "
                   "type {}, but valid_values only allowed for character "
                   "and integer variables.".format(name, type_))
            if type_ == 'integer':
    ...
    def _user_modifiable_in_variable_definition(self, name):
        ...
        if user_modifiable_only_by_xml is not None:
            expect(False,
                   "Cannot change {} in user_nl_xxx file, {}".format(name, user_modifiable_only_by_xml))
        user_cannot_modify = node.get('cannot_modify_by_user_nl')
        if user_cannot_modify is not None:
            expect(False,
                   "Cannot change {} in user_nl_xxx file, {}".format(name, user_cannot_modify))
    ...
    def dict_to_namelist(self, dict_, filename=None):
        ...
        # Improve error reporting when a file name is provided.
        if filename is None:
            variable_template = "Variable {:r}s"
        else:
            variable_template = "Variable {:r} from file " + repr(str(filename))
        groups = {}
